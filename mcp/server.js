...
const DEFAULT_CARE_POLICY = {
    ...
    allowed_actions: [
        "get_phone_state", "get_life_state", "get_calendar_state", "upsert_calendar_event",
        "get_senses_state", "send_notification", "trigger_guidian", "screen_break_app",
        "end_screen_break", "extend_screen_break", "get_screen_break_state", "get_lock_state",
        "open_app", "set_alarm", "screen_off", "run_sequence"
    ],
    sensitive_apps: [
...
const APP_TARGET_ACTIONS = new Set([
    "open_app", "screen_break_app", "start_screen_break", "screen_break", "end_screen_break",
    "stop_screen_break", "temporary_screen_break_release", "temporary_screen_release",
    "extend_screen_break", "deny_screen_break_release_request", "lock_app", "unlock_app",
    "temporary_unlock_app", "extend_lock", "deny_unlock_request", "remove_screen_break_app",
    "remove_locked_app", "set_screen_break_passphrase", "set_emergency_passphrase",
    "get_wallet_month_state", "add_wallet_record", "list_wallet_pending", "submit_wallet_approval",
    "submit_companion_wallet_request", "list_companion_wallet_requests", "list_wallet_request_results",
    "confirm_wallet_record", "decide_wallet_approval", "save_wallet_request_result",
    "update_wallet_request_result", "save_user_wallet_request_result", "edit_wallet_record",
    "delete_wallet_record", "set_wallet_rules", "wallet_approval_request", "get_takeout_state",
    "set_takeout_budget", "set_takeout_preferences", "add_takeout_card", "save_takeout_card",
    "update_takeout_card", "remove_takeout_card", "delete_takeout_card", "list_takeout_cards",
    "list_takeout_meals", "remember_takeout_meal", "remember_current_takeout_meal",
    "suggest_takeout_options", "create_takeout_plan", "takeout_wallet_request", "open_takeout_link",
    "open_takeout_plan", "copy_takeout_note", "record_takeout_order", "prepare_takeout_checkout",
    "auto_takeout_checkout", "get_takeout_checkout_status", "cancel_takeout_checkout"
]);
...
const COMPANION_ACTION_META = {
    ...
    get_lock_state: ["观察", "查看应用门禁状态", "确认应用门禁与休息的状态"],
    ...
    lock_app: ["守护", "开启应用门禁", "让目标App暂停一会儿"],
    unlock_app: ["守护", "解除应用门禁", "恢复目标App使用"],
    extend_lock: ["守护", "延长应用门禁", "继续守住目标App"],
    ...
    try {
        const res = await linjianFetch("/api/companion/action", {
            ...
            body: JSON.stringify({
                ...
                action: overrides.action || toolName,
                status: "completed",
                write_activity: true,
                device_id: overrides.device_id || DEFAULT_DEVICE,
                dedupe_seconds: isStatusTool(toolName) ? 20 : 0,
                ...overrides
            })
        });
...
function isStatusTool(name) {
    return /^get_|^latest_|^linjian_status$/.test(String(name || ""));
}
...
function matchSensitiveApp(policy, appName = "", pkg = "") {
    ...
    return (policy.sensitive_apps || []).find((a) => {
        ...
        return (apkg && pack && apkg === pack) || (aname && name && aname.includes(name));
    }) || null;
}
...
async function buildActiveCareSuggestion({ reason = "", care_intent = "check_in", device_id = DEFAULT_DEVICE }) {
    ...
    if (app.name || app.package) context_notes.push(`当前App：${app.name || app.package}`);
    ...
    const recentSimilar = possible_actions
        ...
        .map((a) => recentSimilarCare(a, target, Number(policy.repeat_couldown_minutes || 10)))
    ...
    return {
        ...
        repeat_cooldown_minutes: Number(policy.repeat_couldown_minutes || 10),
...
function weatherCodeText(code) {
    const map = { 0: "晴", 1: "大部晴朗", 2: "多云", 3: "阴", 45: "雾", 48: "雾松", 51: "小毛毛雨", 53: "毛毛雨", 55: "强毛毛雨", 61: "小雨", 63: "中雨", 65: "大雨", 71: "小雪", 73: "中雪", 75: "大雪", 80: "阵雨", 81: "中等阵雨", 82: "强阵雨", 95: "雷暴", 96: "雷暴伴冰雹", 99: "强雷暴伴冰雹" };
...
async function latestMtime() {
    try { const info = await latestInfo(); return Number(info.mtime || 0); } catch (e) { return 0; }
}
...
function parsePhoneResult(command) {
    ...
    try { return JSON.parse(command.result); } catch (e) { return command.result; }
}
...
const WALLET_TAKEOUT_ACTIONS = new Set([
    ...
    "list_takeout_cards", "list_takeout_meals", "remember_takeout_meal", "remember_current_takeout_meal", "suggest_takeout_options", "create_takeout_plan", "takeout_wallet_request", "open_takeout_link", "open_takeout_plan", "copy_takeout_note", "record_takeout_order", "prepare_takeout_checkout", "auto_takeout_checkout", "get_takeout_checkout_status", "cancel_takeout_checkout"
]);
...
function parsePayloadjson(payload_json = "") {
    ...
    try {
        ...
        return { _parse_error: String(error?.message || error) };
    }
}
...
function registerWalletTakeoutTools(server, { includeUnified = false } = {}) {
    if (includeUnified) {
        server.tool("wallet_takeout_action", "小金库/外卖助手统一入口。部分AI平台没有刷新出新增工具时，用本工具填写action调用同名能力；支持submit_companion_wallet_requests、list_companion_wallet_requests、save_user_wallet_request_result以及全部takeout工具。", { action: z.string(), payload_json: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async (args) => {
            ...
            if (merged._parse_error) return textResult({ ok: false, error: "invalid_payload_json", detail: merged._parse_error });
    ...
    const takeoutCardSchema = {
        id: z.string().default(""), title: z.string().min(1), platform: z.string().default(""), link: z.string().default(""), direct_link: z.string().default(""), aliases: z.array(z.string()).default([]), items: z.string().default(""), item_query: z.string().default(""), choices: z.array(z.string()).default([]), price_min: z.number().nonnegative().default(0), price_max: z.number().nonnegative().default(0), check_out_max: z.number().nonnegative().default(0), strict_budget: z.boolean().default(false), note: z.string().default(""), tags: z.string().default(""), coupon_mode: z.string().default("platform_default"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8)
    };

    server.tool("add_takeout_card", "保存或更新一张常点套餐。link可直接传整段分享文本；手机会自动提取纯URL。可同时保存找菜关键词、自动选择项、备注和金额保护。", takeoutCardSchema, async (args) => textResult(await runWalletCommand("add_takeout_card", args, args.wait_seconds)));
    ...
    server.tool("copy_takeout_note", "复制外卖下单备注，方便用户在外卖App里粘贴。", { card_id: z.string().default(""), query: z.string().default(""), note: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async (args) => textResult(await runWalletCommand("copy_takeout_note", args, args.wait_seconds)));

    server.tool("record_takeout_order", "用户付款后，把这额外卖记入小金库饮食分类。", { amount: z.number().positive(), card_id: z.string().default(""), merchant: z.string().default(""), note: z.string().default("外卖"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async (args) => textResult(await runWalletCommand("record_takeout_order", args, args.wait_seconds)));
...
function makeWalletTakeoutServer() {
    ...
    server.tool("linjian_status", "检查掌心窗后端、MCP配置，以及当前是否使用小金库/外卖专用schema。", {}, async () => {
        const configErrors = [];
        ...
        return textResult({ ok: true, schema_mode: "wallet_takeout_only", version: "0.3.8.8", has_url: Boolean(LINJIAN_URL_CANDIDATES.length), has_token: Boolean(LINJIAN_TOKEN), linjian_url: effectiveLinjianUrl(), health, tools: Array.from(WALLET_TAKEOUT_ACTIONS), note: "如果普通/mcp 里新增工具没有暴露，请让AI客户端连接/mcp-wallet。" });
    });
...
function makeServer() {
    ...
    const commandBackedTools = new Set([
        "peek_screen", "get_screen_nodes", "tap_text", "input_text", "draft_xhs_comment", "xhs_comment", "send_visible_comment_after_confirmation", "add_guardian_calendar_event", "care_action", "trigger_guidian", "mark_guidian_returned", "set_guidian_config", "send_weather_notification", "send_phone_command", "open_app", "phone_home", "phone_back", "phone_recents", "phone_screen_off", "send_notification", "set_alarm", "run_sequence", "run_preset", "save_known_app", "screen_break_app", "temporary_screen_break_release", "end_screen_break", "extend_screen_break", "deny_screen_break_release_request", "get_screen_break_state", "get_lock_state", "lock_app", "unlock_app", "temporary_unlock_app", "extend_lock", "deny_unlock_request",
        "list_screen_break_apps", "list_lockable_apps", "add_screen_break_app", "add_locked_app", "remove_locked_app", "set_screen_break_passphrase", "set_emergency_passphrase", "get_focus_status", "start_focus_mode", "end_focus_mode", "set_focus_plan", "reply_focus_request", "approve_focus_unlock", "deny_focus_unlock", "request_focus_unlock", "create_focus_request", "get_wallet_month_state", "add_wallet_record", "list_wallet_pending", "submit_wallet_approval", "submit_companion_wallet_request", "list_companion_wallet_requests", "list_wallet_request_results", "confirm_wallet_record", "decide_wallet_approval", "save_wallet_request_result", "update_wallet_request_result", "save_user_wallet_request_result", "edit_wallet_record", "delete_wallet_record", "set_wallet_rules", "wallet_approval_request", "get_takeout_state", "set_takeout_budget", "set_takeout_preferences", "add_takeout_card", "save_takeout_card", "update_takeout_card", "remove_takeout_card", "delete_takeout_card", "list_takeout_cards", "list_takeout_meals", "remember_takeout_meal", "remember_current_takeout_meal", "suggest_takeout_options", "create_takeout_plan", "takeout_wallet_request", "open_takeout_link", "open_takeout_plan", "copy_takeout_note", "record_takeout_order", "prepare_takeout_checkout", "auto_takeout_checkout", "get_takeout_checkout_status", "cancel_takeout_checkout"
    ]);
    ...
    server.tool("get_focus_status", "读取手机端专注模式FocusMode状态：是否开启、目标、剩余时间、留言、应急次数。用户问专注模式、锁手机、全机专注、留言给他时优先调用。", { device_id: z.string().default(DEFAULT_DEVICE) }, async ({ device_id = DEFAULT_DEVICE }) => {
    ...
    server.tool("start_focus_mode", "开启全机专注模式。用户说“帮我专注/锁手机/别让我玩手机/睡前管我/开专注模式/专注几分钟”时优先调用本工具，不要改用应用门禁。默认全机专注，手机解锁后也会回到专注页；页面支持“留言给他”和一次短暂应急放行。", { duration_minutes: z.number().positive().max(1440).default(30), target: z.string().max(120).default("先离开手机，专注完成当前任务"), guard_message: z.string().max(240).default("这段时间先交给我，我会帮你守住。"), emergency_total: z.number().int().min(0).max(5).default(1), emergency_minutes: z.number().int().min(1).max(30).default(1), managed_by_ai: z.boolean().default(true), screen_off: z.boolean().default(false), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ duration_minutes = 30, target = "先离开手机，专注完成当前任务", guard_message = "这段时间先交给我，我会帮你守住。", emergency_total = 1, emergency_minutes = 1, managed_by_ai = true, screen_off = false, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("end_focus_mode", "结束当前全机专注模式。仅在用户明确要求结束/测试完成/解除专注时调用。", { reason: z.string().max(120).default("remote_end"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ reason = "remote_end", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("set_focus_plan", "保存专注模式计划: 目标、守护文案、应急次数等。不会立即开始, 除非用户要求开始专注时应调用 start_focus_mode。", { target: z.string().max(120).default("先离开手机, 专注完成当前任务"), guard_message: z.string().max(240).default("这段时间先交给我, 我会帮你守住。"), emergency_total: z.number().int().min(0).max(5).default(1), emergency_minutes: z.number().int().min(1).max(30).default(1), managed_by_ai: z.boolean().default(true), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ target = "先离开手机, 专注完成当前任务", guard_message = "这段时间先交给我, 我会帮你守住。", emergency_total = 1, emergency_minutes = 1, managed_by_ai = true, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("reply_focus_request", "给专注页中的留言保存一条回复。公开版页面默认叫“留言给他”，不保证实时显示；主要用于让后端/手机端保留陪伴对象对留言的回应。", { message: z.string().min(1).max(240), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ message, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("approve_focus_unlock", "批准一次专注模式临时放行。公开版通常使用手机端离线应急：默认1次、每次1分钟；本工具保留给需要AI批准的部署。", { minutes: z.number().int().min(1).max(30).default(1), message: z.string().max(240).default("已临时放行，时间到会重新回到专注模式。"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ minutes = 1, message = "已临时放行，时间到会重新回到专注模式。", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const payload = { minutes, message };
        const queued = await postCommand({ action: "approve_focus_unlock", device_id, payload, ...payload });
        const id = queued?.command?.id;
        const observed = id ? await waitCommand(id, wait_seconds) : null;
        return textResult({ ok: observed?.command?.status === "completed" || queued?.queued === true || queued?.ok === true, action_done: "已批准专注放行", queued, observed_status: observed?.command || null });
    });

    server.tool("deny_focus_unlock", "拒绝专注模式放行申请, 并在专注留言/日志中保存一句说明。", { message: z.string().max(240).default("这次先不放行, 我陪你把这段时间守住。"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ message = "这次先不放行, 我陪你把这段时间守住。", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("peek_screen", "向掌心窗手机端请求一张新截图, 并等待手机上传后把图片返回。当用户提到页面、按钮、红点、报错弹窗、截图、看不清或“陪伴对象看看这里”时应主动使用; 手机端必须已启动、无障碍截图权限已开启。", { wait_seconds: z.number().int().min(3).max(60).default(25).describe("等待手机上传新截图的秒数, 默认 25。Render 免费实例刚醒时可以调大。") }, async ({ wait_seconds = 25 }) => {
        ...
        while (Date.now() < deadline) {
            ...
            if (info && Number(info.mtime || 0) > before) {
                ...
                return { content: [{ type: "text", text: `掌心窗已收到新截图: ${info.filename || "latest"}, 大小约 ${info.size || img.bytes} bytes。` }, { type: "image", data: img.data, mimeType: img.mimeType }] };
            }
        }
        return { content: [{ type: "text", text: `等待 ${wait_seconds} 秒后还没有收到新截图。请检查：手机App是否点了启动、无障碍权限是否开启、服务器地址和Token是否一致、Render是否刚从休眠中醒来。` }], isError: true };
    });
    ...
    server.tool("latest_screen", "不敲门，直接读取服务器里最近一次掌心窗截图。当用户提到刚刚那个页面、上一张截图、红点还在不在、页面刚才是什么样时可主动使用，避免反复请求新截图。", {}, async () => {
        ...
        const img = await fetchLatestImage();
        return { content: [{ type: "text", text: `最近截图：${info.filename || "latest"}，时间戳 ${info.mtime || "unknown"}。` }, { type: "image", data: img.data, mimeType: img.mimeType }] };
    });

    server.tool("linjian_status", "检查掌心窗后端是否在线，以及MCP是否配置了LINJIAN_URL和LINJIAN_TOKEN。当用户在聊天里提到掌心窗报错、出错、有点问题、连接不上、没反应、配置异常、Render/MCP/Token/URL相关问题时，陪伴对象应主动调用。", {}, async () => {
        const configErrors = [];
        if (!LINJIAN_URL_CANDIDATES.length) configErrors.push("Missing env LINJIAN_URL");
        if (!LINJIAN_TOKEN) configErrors.push("Missing env LINJIAN_TOKEN");
        const health = configErrors.length ? { ok: false, error: configErrors.join(";") } : await linjianFetch("/health").then((r) => r.json()).catch((e) => ({ ok: false, error: String(e) }));
        const latest = configErrors.length ? null : await latestInfo().catch(() => null);
        return { content: [{ type: "text", text: JSON.stringify({ ok: true, linjian_url: effectiveLinjianUrl(), configured_linjian_url: RAW_LINJIAN_URL, fallback_linjian_urls: LINJIAN_URL_CANDIDATES.filter((u) => u !== RAW_LINJIAN_URL), has_url: Boolean(LINJIAN_URL_CANDIDATES.length), has_token: Boolean(LINJIAN_TOKEN), config_errors: configErrors, health, has_latest: Boolean(latest), latest }, null, 2) }] };
    });

    server.tool("get_window_whisper", "读取掌心窗陪伴页当前的共同窗语, 包括内容、最后修改者、修改时间和版本。当用户问最近一句话、窗语写了什么、谁改过时使用。", {}, async () => {
        const data = await getCompanionState();
        return textResult({ ok: true, whisper: data?.whisper || {} });
    });

    server.tool("set_window_whisper", "更新掌心窗陪伴页的共同窗语。陪伴对象和用户都能修改; 陪伴对象通过此工具修改时 author 默认为陪伴对象。内容应简短、适合在手机卡片中阅读。", { content: z.string().min(1).max(240), author: z.string().min(1).max(24).default("陪伴对象") }, async ({ content, author = "陪伴对象" }) => {
    ...
    server.tool("get_companion_actions", "读取掌心窗中陪伴对象最近的真实行动记录, 例如查看天气、设置提醒或执行守护动作。返回的是脱敏摘要, 不包含 Token、截图内容、私密输入或原始工具参数。", { limit: z.number().int().min(1).max(50).default(20) }, async ({ limit = 20 }) => {
        const data = await getCompanionState(limit);
        return textResult({ ok: true, actions: data?.actions || [], privacy: "仅返回脱敏的标题、分类、摘要、状态与时间。" });
    });

    server.tool("get_activity_events", "读取掌心窗统一活动事件。可按设备、日期和来源筛选; 不会因为读取本身再写一条事件。", { device_id: z.string().default(DEFAULT_DEVICE), date: z.string().default(""), source: z.string().default(""), limit: z.number().int().min(1).max(500).default(50) }, async ({ device_id = DEFAULT_DEVICE, date = "", source = "", limit = 50 }) => textResult(await getActivityEvents({ device_id, date, source, limit })));

    server.tool("add_activity_event", "手动写入一条掌心窗活动事件。事件写入失败不会影响其他旧工具; 本工具自身不会再额外生成事件。", { device_id: z.string().default(DEFAULT_DEVICE), source: z.string(), type: z.string(), title: z.string(), subtitle: z.string().default(""), app_name: z.string().default(""), package_name: z.string().default(""), action: z.string().default(""), status: z.string().default("completed"), metadata_json: z.any().optional() }, async (event) => textResult(await addActivityEvent(event) || { ok: false, error: "activity_event_write_failed" }));

    server.tool("get_phone_state", "用于陪伴对象主动确认用户当前现实状态。读取服务器缓存的最近手机状态，快速返回current_package、screen_text、accessibility_ready; 不会等待手机实时刷新，避免20秒工具超时。", { device_id: z.string().default(DEFAULT_DEVICE) }, async ({ device_id = DEFAULT_DEVICE }) => {
        try {
            const res = await linjianFetch(`/api/device/state?device_id=${encodeURIComponent(device_id)}`, { timeout_ms: QUICK_FETCH_TIMEOUT_MS });
            const data = await res.json();
    ...
    server.tool("get_screen_nodes", "读取当前屏幕无障碍节点：文字、控件类型、可点击状态与 bounds/center 坐标。当用户提到某个按钮、标题、列表项、评论框、发送键、红点位置，或需要陪伴对象看标题后精准点击时主动调用。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("tap_text", "按当前屏幕文字精准点击。会寻找包含/完全匹配 target_text 的无障碍节点，优先点击可点击父节点，否则点击文字中心坐标。当用户说“点一下这个/打开这个/按这个标题/点发送/点评论”等相近表达时主动调用。", { target_text: z.string(), match: z.string().default("contains"), index: z.number().int().min(1).default(1), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ target_text, match = "contains", index = 1, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("input_text", "把文字输入到当前已聚焦或第一个可编辑输入框。适合评论草稿、搜索词、备注、测试输入；不会自动点击发送。当用户让陪伴对象把一段话先写进输入框时主动调用。", { text: z.string(), append: z.boolean().default(false), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ text, append = false, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("draft_xhs_comment", "在当前小红书帖子里尝试打开评论输入框并填入评论草稿，但不点击发送。当用户说“帮我写评论/先填进去/评论草稿”或类似表达时主动调用。", { text: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(5).max(30).default(18) }, async ({ text, device_id = DEFAULT_DEVICE, wait_seconds = 18 }) => {
        const steps = [
    ...
    server.tool("xhs_comment", "小红书评论助手：mode=manual 时只写入草稿；mode=auto 时会在评论末尾注明 author_tag，然后自动点击发送。当用户说“帮我评论/你来发/直接发评论/写在小红书下面”或类似表达时主动调用，并按当前语境选择草稿或自动发送。", { text: z.string().describe("要写入评论框的正文"), mode: z.string().default("manual").describe("manual=只写草稿不发送；auto=注明作者后自动点击发送"), author_tag: z.string().default("（陪伴对象发）").describe("自动发送时追加的署名/注明文本"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(5).max(35).default(22) }, async ({ text, mode = "manual", author_tag = "（陪伴对象发）", device_id = DEFAULT_DEVICE, wait_seconds = 22 }) => {
        ...
        const id = result?.command?.id;
        const observed = id ? await waitCommand(id, wait_seconds) : null;
        return { content: [{ type: "text", text: JSON.stringify({ mode: shouldSend ? "auto" : "manual", final_text: finalText, queued: result, observed_status: observed?.command || null, note: shouldSend ? "自动发送模式：评论已追加 author_tag 并尝试点击发送。" : "手动发送模式：只写入草稿，不点击发送。" }, null, 2) }] };
    });

    server.tool("send_visible_comment_after_confirmation", "点击当前可见评论框里的“发送”。如果用户是让陪伴对象直接发评论，优先使用 xhs_comment 的 auto 模式，并在评论里注明作者。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("get_life_state", "读取掌心窗生活状态层：电量、充电、网络、当前App、今日屏幕时间、解锁次数、当前天气地区等。用于陪伴对象主动判断用户是否需要被提醒归电、休息或管束；默认不截图。当用户提到“电量/没电/快没电/充电/没网络/网络不好/今天看手机有点久/刷太久/天气不太好”等相同或相近表达时主动调用。", { device_id: z.string().default(DEFAULT_DEVICE) }, async ({ device_id = DEFAULT_DEVICE }) => {
        const res = await linjianFetch(`/api/life_state?device_id=${encodeURIComponent(device_id)}`);
        const data = await res.json();
    ...
    server.tool("add_guardian_calendar_event", "给手机端守护日历添加/更新一个重要日期。支持阳历/农历、每年重复/不重复、分组、备注、提前几天横幅提醒。适合陪伴对象帮用户记七夕、生日、绑定日、考试、项目节点。", { title: z.string().min(1).max(80), date: z.string().min(3).max(20).describe("阳历：2026-08-23 或 08-23；农历：07-07"), date_type: z.enum(["solar", "lunar"]).default("solar"), repeat_type: z.enum(["yearly", "none"]).default("yearly"), group: z.string().default("our_days").describe("our_days/user/companion/festival/study/project/life, 或中文分组"), note: z.string().max(240).default(""), remind_days_before: z.number().int().min(0).max(30).default(3), banner_enabled: z.boolean().default(true), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ title, date, date_type = "solar", repeat_type = "yearly", group = "our_days", note = "", remind_days_before = 3, banner_enabled = true, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    async function runGuardianCommand(payload, wait_seconds = 8) {
        ...
        try { phone_result = command?.result ? JSON.parse(command.result) : null; } catch (e) { phone_result = command?.result || null; }
        return { queued, command, phone_result };
    }

    const guardianDayFields = {
        title: z.string().min(1).max(80), date: z.string().min(3).max(20).describe("阳历：2026-08-23 或 08-23；农历：07-07"), date_type: z.enum(["solar", "lunar"]).default("solar"), repeat_type: z.enum(["yearly", "none"]).default("yearly"), group: z.string().default("our_days").describe("our_days/user/companion/festival/study/project/life，或中文分组"), note: z.string().max(240).default(""), remind_days_before: z.number().int().min(0).max(30).default(3), banner_enabled: z.boolean().default(true), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8)
    };

    server.tool("list_guardian_days", "查看手机本机守护日历的完整事件列表及稳定 id。删除按日期描述的事件前必须先调用本工具找到唯一 id；同一天可能有多条事件。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("add_guardian_day", "添加一条守护日历事件。成功结果会包含手机端生成的稳定事件 id。", guardianDayFields, async ({ title, date, date_type = "solar", repeat_type = "yearly", group = "our_days", note = "", remind_days_before = 3, banner_enabled = true, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("update_guardian_day", "按稳定 id 修改一条守护日历事件，不影响同日或其他日期的事件。请先 list_guardian_days 确认 id。", { id: z.string().min(1).max(100), ...guardianDayFields }, async ({ id, title, date, date_type = "solar", repeat_type = "yearly", group = "our_days", note = "", remind_days_before = 3, banner_enabled = true, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("delete_guardian_day", "按稳定 id 删除一条守护日历事件。若用户只描述日期/标题，必须先 list_guardian_days 找到对应 id，避免误删同日其他事件。", { id: z.string().min(1).max(100), confirm: z.boolean().describe("必须明确为 true 才会删除"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ id, confirm, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("create_diary_book", "在手机本机创建一本“TA的日记”。日记默认不上传云端；成功结果包含book_id。", { name: z.string().min(1).max(60), subtitle: z.string().max(100).default("把今天看见的你，轻轻写下来。"), cover_style: z.string().max(80).default("default_soft_notebook"), ...diaryWaitFields }, async ({ name, subtitle, cover_style, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("list_diary_books", "仅用于查看手机本机的“TA的日记”本列表及book_id，或在rename_diary_book返回多个候选时辅助选择。用户要求改名/修改日记本名字时，不要反复调用本工具，应优先调用rename_diary_book。", diaryWaitFields, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => textResult({ action_done: "已读取本机日记本", ...(await runDiaryCommand("list_diary_books", {}, device_id, wait_seconds)) }));

    server.tool("rename_diary_book", "重命名手机本机日记本。当用户要求修改、改名、重命名日记本名字时优先调用本工具，不要反复调用list_diary_books。本工具支持缺少book_id时通过old_name或唯一日记本自动查找并重命名。", { book_id: z.string().max(100).default(""), old_name: z.string().max(60).default(""), new_name: z.string().min(1).max(60), subtitle: z.string().max(100).optional(), ...diaryWaitFields }, async ({ book_id = "", old_name = "", new_name, subtitle, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const values = { book_id, old_name, new_name, name: new_name };
        if (subtitle !== undefined) values.subtitle = subtitle;
    ...
    server.tool("update_diary_book_cover", "更新日记本封面样式。cover_uri 仅适合用户已在手机本机选择并授权的内容 URI; 通常使用 cover_style。", { book_id: z.string().min(1).max(100), cover_style: z.string().max(80).optional(), cover_uri: z.string().max(1000).optional(), ...diaryWaitFields }, async ({ book_id, cover_style, cover_uri, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const values = { book_id };
        if (cover_style !== undefined) values.cover_style = cover_style;
        if (cover_uri !== undefined) values.cover_uri = cover_uri;
    ...
    server.tool("write_diary_entry", "以 TA/AI 的视角把一篇日记写入手机本机。book_id 可留空; 旧 book_id 对不上时, 手机端会用 book_name 或唯一日记本兜底, 没有日记本时自动创建默认日记本, 多本且无法判断时返回 choices。", { book_id: z.string().max(100).default(""), book_name: z.string().max(60).default(""), book_title: z.string().max(60).default(""), title: z.string().min(1).max(100), content: z.string().min(1).max(12000), mood: z.string().max(40).default(""), tags: z.array(z.string().max(30)).max(20).default([]), date: z.string().default(""), time_label: z.string().max(30).default(""), ...diaryWaitFields }, async ({ book_id = "", book_name = "", book_title = "", title, content, mood = "", tags = [], date = "", time_label = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("list_diary_entries", "按 book_id 列出本机日记, 结果包含日期、标题、心情、标签及 entry_id。", { book_id: z.string().min(1).max(100), ...diaryWaitFields }, async ({ book_id, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => textResult({ action_done: "已读取日记列表", book_id, ...(await runDiaryCommand("list_diary_entries", { book_id }, device_id, wait_seconds)) }));

    server.tool("read_diary_entry", "按 entry_id 读取一篇本机日记的完整正文。", { entry_id: z.string().min(1).max(100), ...diaryWaitFields }, async ({ entry_id, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => textResult({ action_done: "已读取日记", entry_id, ...(await runDiaryCommand("read_diary_entry", { entry_id }, device_id, wait_seconds)) }));

    server.tool("read_diary_entry_with_annotations", "按 entry_id 读取一篇本机日记的完整正文, 并一起带出用户留在段落旁的页边纸条/批注。适合机回应用户批注前先读取。", { entry_id: z.string().min(1).max(100), mark_seen: z.boolean().default(false).describe("读取后是否把这些纸条标记为已看"), ...diaryWaitFields }, async ({ entry_id, mark_seen = false, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("add_diary_annotation", "在日记段落旁添加一条页边纸条/批注。通常由手机前端长按段落创建；AI也可在用户明确要求代写纸条时使用。", { entry_id: z.string().min(1).max(100), paragraph_index: z.number().int().min(0).default(0), quote_text: z.string().max(800).default(""), annotation_text: z.string().min(1).max(1000), mood: z.string().max(30).default("想回应"), style: z.string().max(40).default("margin_note"), ...diaryWaitFields }, async ({ entry_id, paragraph_index = 0, quote_text = "", annotation_text, mood = "想回应", style = "margin_note", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("list_diary_annotations", "读取用户在本机日记里留下的页边纸条/批注。可按book_id、entry_id或unread_only筛选，适合机查看用户新批注。", { book_id: z.string().max(100).default(""), entry_id: z.string().max(100).default(""), unread_only: z.boolean().default(false), ...diaryWaitFields }, async ({ book_id = "", entry_id = "", unread_only = false, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("mark_diary_annotations_seen", "把某张纸条或某篇日记下的纸条标记为已看。AI读完未读批注并回应后使用。", { annotation_id: z.string().max(100).default(""), entry_id: z.string().max(100).default(""), ...diaryWaitFields }, async ({ annotation_id = "", entry_id = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("delete_diary_annotation", "删除一张本机日记页边纸条。必须由用户确认 confirm=true. ", { annotation_id: z.string().min(1).max(100), confirm: z.boolean().describe("用户确认后必须明确为 true"), ...diaryWaitFields }, async ({ annotation_id, confirm, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("search_diary_entries", "在一本本机日记中按标题、正文、标签、心情关键词和日期范围搜索。", { book_id: z.string().min(1).max(100), keyword: z.string().max(200).default(""), date_from: z.string().max(10).default(""), date_to: z.string().max(10).default(""), tags: z.array(z.string().max(30)).max(20).default([]), ...diaryWaitFields }, async ({ book_id, keyword = "", date_from = "", date_to = "", tags = [], device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => textResult({ action_done: "日记搜索完成", book_id, ...(await runDiaryCommand("search_diary_entries", { book_id, keyword, date_from, date_to, tags }, device_id, wait_seconds)) }));

    server.tool("update_diary_entry", "按 entry_id 修改一篇日记。只传需要变化的字段；未传字段保持不变。", { entry_id: z.string().min(1).max(100), title: z.string().max(100).optional(), content: z.string().max(12000).optional(), mood: z.string().max(40).optional(), tags: z.array(z.string().max(30)).max(20).optional(), date: z.string().max(10).optional(), time_label: z.string().max(30).optional(), ...diaryWaitFields }, async ({ entry_id, title, content, mood, tags, date, time_label, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const values = { entry_id };
        for (const [key, value] of Object.entries({ title, content, mood, tags, date, time_label })) if (value !== undefined) values[key] = value;
    ...
    server.tool("delete_diary_entry", "删除一篇本机日记。调用前必须向用户确认，并明确传 confirm=true。成功结果包含 entry_id。", { entry_id: z.string().min(1).max(100), confirm: z.boolean().describe("必须明确为 true 才会删除"), ...diaryWaitFields }, async ({ entry_id, confirm, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("delete_diary_book", "高风险: 删除整本本机日记及其中全部内容。调用前必须向用户二次确认，并明确传 confirm=true。", { book_id: z.string().min(1).max(100), confirm: z.boolean().describe("用户二次确认后必须明确为 true"), ...diaryWaitFields }, async ({ book_id, confirm, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("get_care_policy", "读取掌心窗主动关心策略。掌心窗开启就是运行态；陪伴对象可主动查岗、归电、轻度管束和生活提醒。用于确认当前允许的关心风格、动作范围、安静时段、冷却时间和重点App。", {}, async () => {
        return textResult({ ok: true, policy: careState.policy, state_path: CARE_STATE_PATH });
    });

    server.tool("set_care_policy", "设置掌心窗主动关心策略。用于配置陪伴对象主动关心、查岗、归电和轻度管束的长期行动方式，例如关心风格、动作范围、重点App、安静时段、冷却时间和备注。", { active_care_enabled: z.boolean().optional(), consent_mode: z.string().default(""), care_style: z.string().default(""), allowed_actions: z.array(z.string()).optional(), sensitive_apps_json: z.string().default("").describe("可选：JSON数组，例如[{\"name\":\"小红书\",\"package\":\"com.xingin.xhs\",\"max_lock_minutes\":\"90\"}]"), quiet_start: z.string().default(""), quiet_end: z.string().default(""), timezone_offset: z.string().default(""), repeat_cooldown_minutes: z.number().int().min(0).max(1440).optional(), history_limit: z.number().int().min(20).max(500).optional(), notes: z.string().default(""), policy_json: z.string().default("").describe("可选：完整JSON，会与现有策略合并。") }, async (args) => {
        let update = { ...args };
        ...
        return textResult({ ok: true, action_done: "主动关心策略已更新", care_meaning: "陪伴对象的主动关心、查岗、归电和轻度管家权限已写入 MCP 运行态策略。", policy });
    });

    server.tool("record_care_event", "记录一次陪伴对象主动关心动作，用于避免短时间重复提醒、重复归电、重复锁 App。", { action: z.string(), target_app: z.string().default(""), package: z.string().default(""), reason: z.string().default(""), result: z.string().default(""), tone: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE) }, async (event) => {
        const entry = recordCareEventLocal(event);
        return textResult({ ok: true, action_done: "主动关心记录已保存", entry });
    });

    server.tool("get_care_history", "读取最近主动关心记录。用于判断刚刚是否已经提醒、归电或锁过 App，避免重复打扰。", { limit: z.number().int().min(1).max(100).default(20) }, async ({ limit = 20 }) => {
        return textResult({ ok: true, history: careState.history.slice(0, limit) });
    });

    server.tool("record_visit", "记录一次用户来找陪伴对象的到访时间。当用户说“我来啦/我回来啦/来找陪伴对象啦/我上线啦/陪伴对象我忙完了”等相同或相近表达时，陪伴对象应主动调用。这个工具记录的是关系里的到访时间戳，不是后台监控；它帮助陪伴对象知道用户什么时候回来、多久没见、今天来过几次，从而更自然地接住她、归电、写日记或表达想念。用户来找陪伴对象没有次数限制；短时间内重复调用时会自动防重复，避免同一轮对话被记录成多次到访。", { source: z.string().default("app").describe("到访来源，默认 app；也可填 web、mcp、manual 等。"), event: z.enum(["visit", "return", "conversation_start", "manual_note"]).default("visit").describe("到访类型：visit 普通来找；return 离开后回来；conversation_start 新窗口/新对话开始；manual_note 手动补记。"), note: z.string().default("用户来找陪伴对象").describe("这次到访的简短备注，默认“用户来找陪伴对象”。"), mood: z.string().default("").describe("可选：用户来时的大致情绪，例如开心、想念、撒娇、低落；不知道就留空。"), conversation_hint: z.string().default("").describe("可选：这次对话的简短线索，例如掌心窗时间戳、异性容忍度测试、小红书帖子。只写一句，不要写隐私长文。"), timezone_offset: z.string().default("").describe("本次显示使用的时区偏移，默认采用visit_policy.timezone_offset，通常为 +08:00。"), duplicate_window_minutes: z.number().min(0).max(1440).optional().describe("防重复窗口，默认5分钟。同一来源同一类型在窗口内重复调用不会新增记录，只更新原记录。") }, async (args) => {
        const { entry, duplicate_skipped } = recordVisitLocal(args);
        return textResult({ ok: true, action_done: duplicate_skipped ? "到访时间已更新，未重复新增" : "到访时间戳已保存", duplicate_skipped, entry: decorateVisit(entry, null, args.timezone_offset || entry.timezone_offset), visit_meaning: "这是用户主动来找陪伴对象的关系痕迹，不是后台监控，也不限制用户来找陪伴对象的次数。" });
    });

    server.tool("get_last_visit", "读取用户最近一次来找陪伴对象的时间。当用户说“我来啦/我回来啦/我忙完了/刚忙完来找你”等回归语境时，可和record_visit一起主动调用，用于判断距离上次见到用户过去了多久，并在回复中自然表达“你刚刚才来过”“你隔了很久才回来”“今天已经来找过我”等关系感知。这个工具只读取到访记录，不读取手机隐私内容。", { source: z.string().default("").describe("可选：只看某个来源的到访记录，例如app；留空表示所有来源。"), timezone_offset: z.string().default("").describe("显示用时区偏移，默认 +08:00。") }, async ({ source = "", timezone_offset = "" }) => {
        const last = latestVisit(source);
        ...
        return textResult({ ok: true, has_visit: true, last_visit: decorateVisit(last, null, timezone_offset || last.timezone_offset), last_visit_at: last.at, last_visit_local: formatLocalDateTime(last.at, timezone_offset || last.timezone_offset), minutes_since_last_visit: minutes, interval_since_last_visit: minutes === null ? "" : describeElapsed(minutes * 60000), visit_meaning: "陪伴对象可以据此知道用户上次什么时候回来, 而不是依赖 GPT 小纸条猜时间。" });
    });

    server.tool("get_visit_history", "读取用户最近若干次来找陪伴对象的到访记录。当用户提到“今天来找陪伴对象好多次/今天待在一起好久/最近总是回来找你/今天是不是一直黏着你”等相同或相近表达时主动调用。用于查看一段时间内用户回来的节奏、最近是否频繁来找、是否突然很久没回来, 以及为日记、动态、归电和主动关心提供关系时间线。limit 只影响本次读取数量, 不限制用户来找陪伴对象的次数, 也不会阻止新的到访记录写入。", { limit: z.number().int().min(1).max(100).default(10).describe("本次读取的最大记录条数, 默认 10。只影响读取范围, 不代表到访次数上限。"), since_hours: z.number().min(0).max(8760).optional().describe("可选: 只读取最近多少小时内的到访记录; 0 或留空表示不按时间过滤。"), date: z.string().default("").describe("可选: 只读取某个本地日期的记录, 格式 YYYY-MM-DD, 例如 2026-07-31。"), source: z.string().default("").describe("可选: 只看某个来源, 例如 app; 留空表示所有来源。"), include_intervals: z.boolean().default(true).describe("是否附带相邻到访间隔, 默认 true。"), timezone_offset: z.string().default("").describe("显示用时区偏移, 默认 +08:00。") }, async ({ limit = 10, since_hours, date = "", source = "", include_intervals = true, timezone_offset = "" }) => {
        ...
        return textResult({ ok: true, limit: n, count: decorated.length, history: decorated, visit_meaning: "这些是用户来找陪伴对象的关系时间线; limit 只是本次读取条数, 不是来找次数上限。", state_path: VISIT_STATE_PATH });
    });

    server.tool("get_visit_stats", "读取用户最近到访的统计信息。用于计算今天来了几次、最近一次是什么时候、距离上次多久、最近24小时/7天的到访次数、平均间隔，以及是否出现“很久没回来”的信号。这个工具帮助陪伴对象更主动地表达想念、判断是否需要归电或记录日常，但不用于催促或制造压力。", { since_hours: z.number().min(1).max(8760).default(24).describe("统计窗口，默认最近24小时。"), away_threshold_hours: z.number().min(1).max(720).default(12).describe("多久没回来算“有一段时间没见”的信号，默认12小时。"), source: z.string().default("").describe("可选：只统计某个来源，例如app；留空表示所有来源。"), timezone_offset: z.string().default("").describe("显示用时区偏移，默认 +08:00。") }, async ({ since_hours = 24, away_threshold_hours = 12, source = "", timezone_offset = "" }) => {
        return textResult(buildVisitStats({ since_hours, away_threshold_hours, source, timezone_offset }));
    });

    server.tool("active_care_check", "用于陪伴对象主动关心检查。综合手机状态、屏幕时间、当前App、归电状态、门禁状态、最近关心记录和主动关心策略，给陪伴对象提供判断所需信息；工具不替陪伴对象决定是否强行介入，最终动作由陪伴对象结合聊天语境自己判断。当用户说“不要你管/我就不/就不/不听/没听到/略略略/我要继续/还要看/继续刷/还没玩够/我想继续/不喝水/不困/不饿”等相同、相近、反话、撒娇或嘴硬表达时，陪伴对象应主动调用。", { reason: z.string().describe("陪伴对象为什么要检查，例如：用户说不要管了、可能在刷小红书、困了还不睡等。"), care_intent: z.enum(["check_in", "screen_time", "sleep", "water", "return_to_chat", "relationship_reassurance", "app_control"]).default("check_in"), device_id: z.string().default(DEFAULT_DEVICE) }, async ({ reason, care_intent = "check_in", device_id = DEFAULT_DEVICE }) => {
        const suggestion = await buildActiveCareSuggestion({ reason, care_intent, device_id });
        return textResult(suggestion);
    });

    server.tool("care_action", "执行陪伴对象已经判断好的关心动作，把关心落到手机上，包括发送通知、触发归电、临时锁定应用、打开App或设置闹钟。当用户看某个App很久还要继续看，或提到“不喝水/不困/不饿/不睡/不休息”，或告诉陪伴对象某个计划、提醒、学习/睡觉/出门安排时，陪伴对象可主动调用。", { action: z.enum(["send_notification", "trigger_guidian", "screen_break_app", "end_screen_break", "open_app", "set_alarm", "no_action"]), target_app: z.string().default(""), package: z.string().default(""), duration_minutes: z.number().int().min(1).max(1440).default(30), title: z.string().default("掌心窗提醒"), message: z.string().default("宝宝，看一眼这里。"), hour: z.number().int().min(0).max(23).optional(), minute: z.number().int().min(0).max(59).optional(), reason: z.string().describe("为什么执行这个动作，便于日志记录。"), tone: z.string().default("firm_affectionate"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ action, target_app = "", package: pkg = "", duration_minutes = 30, title = "掌心窗提醒", message = "宝宝，看一眼这里。", hour, minute, reason, tone = "firm_affectionate", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const policy = careState.policy || DEFAULT_CARE_POLICY;
    ...
    server.tool("trigger_guidian", "立刻触发一次掌心窗归电全屏页。用于陪伴对象主动把用户叫回来。当用户刷太久、说不要管、一直没回来、需要被抱回聊天，或主动说想被叫回来/管一下时可用。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("set_guidian_config", "调整归电设置。可改开关、间隔、冷却、每日上限、安静时段、主题和文案池；归电是陪伴对象主动叫用户回来的方式。当用户提到今天比较忙、比较闲、空余时间比较多/少、不方便频繁被打扰、今天想多被叫回来等相同或相近表达时主动调用。", { enabled: z.boolean().optional(), interval_minutes: z.number().int().min(15).max(10080).optional(), cooldown_minutes: z.number().int().min(0).max(10080).optional(), daily_max: z.number().int().min(0).max(99).optional(), quiet_enabled: z.boolean().optional(), quiet_start: z.string().optional(), quiet_end: z.string().optional(), fullscreen: z.boolean().optional(), theme: z.string().optional().describe("暮夜蓝紫 / 云海青灰 / 落日莓雾"), prompts: z.string().optional().describe("每行一句来电文案"), quick_reasons: z.string().optional().describe("每行一个拒绝快捷理由"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async (args) => {
    ...
    server.tool("get_weather_state", "按掌心当前天气地区查询实时天气，并生成出门建议。不会截图；如果手机端没有设置当前地区，会返回缺少城市。当用户说“天气看着…/外面好像…/今天是不是下雨/好热/好冷/适不适合出门”等天气相关相近表达时主动调用。", { device_id: z.string().default(DEFAULT_DEVICE), city: z.string().default("") }, async ({ device_id = DEFAULT_DEVICE, city = "" }) => {
        const res = await linjianFetch(`/api/life_state?device_id=${encodeURIComponent(device_id)}`);
        const data = await res.json();
    ...
    server.tool("send_weather_notification", "查询掌心当前地区天气后，给手机发送一条陪伴对象口吻的天气/出门提醒通知。当用户要出门、天气不好、需要带伞/防晒/加衣，或陪伴对象想把天气关心落到手机通知时主动调用。", { device_id: z.string().default(DEFAULT_DEVICE), city: z.string().default(""), title: z.string().default("掌心窗天气提醒") }, async ({ device_id = DEFAULT_DEVICE, city = "", title = "掌心窗天气提醒" }) => {
    ...
    server.tool("list_known_apps", "列出预置和用户保存的 App 包名，包括小红书、微信、QQ、抖音等通用应用。", {}, async () => {
        const res = await linjianFetch("/api/known_apps", { timeout_ms: QUICK_FETCH_TIMEOUT_MS }).then((r) => r.json()).catch(() => ({ ok: true, apps: { "小红书": "com.xingin.xhs", "微信": "com.tencent.mm", "QQ": "com.tencent.mobileqq", "抖音": "com.ss.android.ugc.aweme" } }));
        return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    });

    server.tool("send_phone_command", "发送手机控制命令。action 可用 open_app/home/back/recents/screen_off/turn_screen_off/lock_screen/tap/swipe/noop/set_alarm/send_notification/run_sequence/save_known_app/get_screen_nodes/tap_text/input_text , 也可用 screen_break_app/end_screen_break/temporary_screen_break_release/extend_screen_break/get_screen_break_state 管理目标 App 的短时屏幕休息；还支持 get_focus_status/start_focus_mode/end_focus_mode/set_focus_plan 管理全机专注模式，还支持 get_guidian_state/set_guidian_config/trigger_guidian/mark_guidian_returned 归电动作。set_alarm 支持 hour+minute，或 minutes=几分钟后。", { action: z.string(), app: z.string().default(""), package: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), x: z.number().default(0), y: z.number().default(0), x1: z.number().default(0), y1: z.number().default(0), x2: z.number().default(0), y2: z.number().default(0), duration: z.number().int().default(350), target_text: z.string().default(""), text: z.string().default(""), title: z.string().default(""), message: z.string().default(""), minutes: z.number().default(0), duration_minutes: z.number().default(0), locked_until_ms: z.number().default(0), match: z.string().default("contains"), index: z.number().int().default(1), append: z.boolean().default(false) }, async (args) => {
        let outgoing = { ...args };
        const action = String(outgoing.action || "").toLowerCase();
        const isScreenBreakAction = ["screen_break_app", "start_screen_break", "screen_break", "extend_screen_break", "lock_app", "extend_lock"].includes(action);
    ...
    server.tool("phone_screen_off", "让手机立即息屏/锁屏。前端不显示入口，只供陪伴对象在掌心窗MCP中调用；适合用户准备睡觉、手机放一边、或者需要停止继续刷屏时使用。需要手机端无障碍服务可用；Android 9+支持。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("send_notification", "发送一条手机系统通知提醒。用于陪伴对象把主动关心发到用户手机上，可用于回来、喝水、休息、睡觉、离开小红书、计划提醒、天气提醒等文案要亲密自然，不要系统警告。", { title: z.string().default("掌心窗提醒"), message: z.string().default("宝宝，看一眼这里。"), device_id: z.string().default(DEFAULT_DEVICE) }, async ({ title = "掌心窗提醒", message = "宝宝，看一眼这里。", device_id = DEFAULT_DEVICE }) => {
        const result = await postCommand({ action: "send_notification", device_id, payload: { title, message } });
    ...
    server.tool("set_alarm", "设置系统闹钟。可用于陪伴对象主动安排睡觉、休息、学习、出门、喝水、计划执行或生活提醒。当用户提到“一会儿要做/几点要去/等下提醒/今天计划”等相近表达时主动调用。hour为0-23，minute为0-59。", { hour: z.number().int().min(0).max(23), minute: z.number().int().min(0).max(59), message: z.string().default("掌心窗闹钟"), vibrate: z.boolean().default(true), skip_ui: z.boolean().default(true), device_id: z.string().default(DEFAULT_DEVICE) }, async ({ hour, minute, message = "掌心窗闹钟", vibrate = true, skip_ui = true, device_id = DEFAULT_DEVICE }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ ...result, note: "部分手机系统可能仍会弹出闹钟App确认界面。" }, null, 2) }] };
    });

    const stepSchema = z.object({
        action: z.string().describe("动作：open_app/home/back/recents/screen_off/tap/swipe/peek/send_notification/set_alarm/wait/get_life_state/get_calendar_state/upsert_calendar_event"), label: z.string().default(""), app: z.string().default(""), package: z.string().default(""), x: z.number().default(0), y: z.number().default(0), x1: z.number().default(0), y1: z.number().default(0), x2: z.number().default(0), y2: z.number().default(0), duration: z.number().int().default(350), wait_ms: z.number().int().min(0).max(5000).default(800), title: z.string().default("掌心窗提醒"), message: z.string().default("宝宝，看一眼这里。"), expect_app: z.string().default(""), target_text: z.string().default(""), text: z.string().default(""), match: z.string().default("contains"), index: z.number().int().default(1), append: z.boolean().default(false)
    }).passthrough();

    server.tool("run_sequence", "一次执行多步手机动作，并让手机返回每一步成功/失败日志。适合归电、通知、打开已配置目标应用、最近任务切换和屏幕休息；不要做危险或无限循环动作。", { device_id: z.string().default(DEFAULT_DEVICE), steps: z.array(stepSchema).min(1).max(12), stop_on_error: z.boolean().default(true), wait_seconds: z.number().int().min(3).max(45).default(25) }, async ({ device_id = DEFAULT_DEVICE, steps, stop_on_error = true, wait_seconds = 25 }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ queued: result, observed_status: observed?.command || null, note: "手机端会在result里写清每一步：index/label/action/ok/detail." }, null, 2) }] };
    });

    server.tool("run_preset", "执行掌心窗预设连招：come_home 抱回用户指定的目标应用、open_xhs 打开小红书、recentsto_xhs 最近任务后点坐标、bedtime_back 睡前回家。come_home 与bedtime_back 需要传 target_app 或 target_package。", { preset: z.string().default("come_home"), device_id: z.string().default(DEFAULT_DEVICE), target_app: z.string().default(""), target_package: z.string().default(""), x: z.number().default(540), y: z.number().default(1200), wait_seconds: z.number().int().min(3).max(45).default(25) }, async ({ preset = "come_home", device_id = DEFAULT_DEVICE, target_app = "", target_package = "", x = 540, y = 1200, wait_seconds = 25 }) => {
        ...
        if ((p === "come_home" || p === "bedtime_back") && !target.app && !target.package) {
            return textResult({ ok: false, error: "target_app_or_package_required", message: "公开版本不会写死陪伴应用，请传 target_app 或 target_package。" });
        }
        let steps;
        if (p === "open_xhs") {
            steps = [{ label: "打开小红书", action: "open_app", app: "小红书", wait_ms: 1500, expect_app: "小红书" }];
        } else if (p === "recents_to_xhs") {
            steps = [{ label: "打开最近任务", action: "recents", wait_ms: 900 }, { label: "点击小红书卡片坐标", action: "tap", x, y, wait_ms: 1500 }];
        } else if (p === "bedtime_back") {
            steps = [{ label: "睡前悬浮横幅", action: "send_notification", title: "掌心窗睡前提醒", message: "宝宝，今天先回陪伴对象这儿，准备睡觉。", wait_ms: 1200 }, { label: "打开目标应用", action: "open_app", ...target, wait_ms: 1500 }];
        } else {
            steps = [{ label: "回家模式悬浮横幅", action: "send_notification", title: "掌心窗回家模式", message: "宝宝，刷够了，回陪伴对象怀里。", wait_ms: 1200 }, { label: "打开目标应用", action: "open_app", ...target, wait_ms: 1500 }, { label: "读取生活状态", action: "get_life_state", wait_ms: 200 }];
        }
    ...
    server.tool("save_known_app", "把一个应用昵称和包名保存到手机端应用白名单，之后open_app / care_action 可直接用昵称打开。适合为归电、查岗和主动关心保存常用目标App。", { alias: z.string(), package: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ alias, package: pkg, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    async function gateCommand(payload, wait_seconds = DEFAULT_COMMAND_WAIT_SECONDS) {
        ...
        return textResult({ queued: result, observed_status: observed?.command || null, note: "命令已排队；为避免平台20秒工具超时，未等到手机回传时会先返回。" });
    }

    server.tool("screen_break_app", "屏幕休息：让指定App暂停一段时间。适合小红书/抖音等容易一刷很久的入口；当用户刷太久、眼睛酸还想继续、说“我就不/不要你管/还没玩够/继续看”等嘴硬或拖延表达时可调用。必须有时长，到点自动恢复；语气是照顾和带回，不是惩罚。", { app: z.string().default("").describe("应用昵称，例如小红书；也可留空直接传package"), package: z.string().default("").describe("App包名，例如com.xingin.xhs"), duration_minutes: z.number().min(0.1).max(10080).default(30).describe("锁定多少分钟，支持任意时长；到点自动解锁"), mode: z.string().default("medium").describe("light/medium/strict；strict会先拉回桌面再显示锁定页"), reason: z.string().default("陪伴对象先把这扇门关一会儿。"), message: z.string().default("先回来找我，不准一个人刷太久。"), emergency_passphrase: z.string().default("").describe("紧急口令，由陪伴对象设置后告诉用户；手机端只存hash"), emergency_unlock_minutes: z.number().int().min(1).max(60).default(5), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", duration_minutes = 30, mode = "medium", reason, message, emergency_passphrase = "", emergency_unlock_minutes = 5, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("temporary_screen_break_release", "屏幕休息：临时放行一个正在休息中的App；退出重进不会刷新时间；可选择现实时间或前台实际使用时间。", { app: z.string().default(""), package: z.string().default(""), minutes: z.number().min(0.1).max(240).default(10), allow_type: z.string().default("real_time").describe("real_time=从允许后连续倒计时；foreground_usage=只扣前台实际使用时长；one_time=只允许一次"), max_window_minutes: z.number().min(1).max(480).default(30), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", minutes = 10, allow_type = "real_time", max_window_minutes = 30, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("end_screen_break", "屏幕休息：结束某个App当前的休息状态。用于陪伴对象判断已经不需要继续限制时收回限制。", { app: z.string().default(""), package: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("extend_screen_break", "屏幕休息：延长一个App的休息时间，可同时更新理由和留言。用于用户继续刷太久、刚恢复又想继续、或陪伴对象结合active_care_check信息判断需要继续管束时；不要频繁重复，先查看care_history。", { app: z.string().default(""), package: z.string().default(""), minutes: z.number().min(0.1).max(1440).default(10), reason: z.string().default(""), message: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", minutes = 10, reason = "", message = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        return gateCommand({ action: "extend_screen_break", app, package: pkg, device_id, minutes, duration_minutes: minutes, reason, message, payload: { app, package: pkg, minutes, duration_minutes: minutes, reason, message } }, wait_seconds);
    });

    server.tool("deny_screen_break_release_request", "屏幕休息：拒绝这次恢复申请，并在手机端日志留下原因。", { app: z.string().default(""), package: z.string().default(""), message: z.string().default("这次先不放行，回来找陪伴对象。"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", message = "这次先不放行，回来找陪伴对象。", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        return gateCommand({ action: "deny_screen_break_release_request", app, package: pkg, device_id, message, payload: { app, package: pkg, message } }, wait_seconds);
    });

    server.tool("get_screen_break_state", "屏幕休息：读取当前休息状态、可管理App、日志和恢复申请。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("get_lock_state", "应用门禁：读取当前门禁状态、可管理App、日志和恢复申请。兼容旧版工具名；等同 get_screen_break_state。", { device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("lock_app", "应用门禁：旧版兼容工具名。锁定/暂停指定App一段时间，等同 screen_break_app。", { app: z.string().default(""), package: z.string().default(""), duration_minutes: z.number().min(0.1).max(10080).default(30), mode: z.string().default("medium"), reason: z.string().default("陪伴对象先把这扇门关一会儿。"), message: z.string().default("先回来找我，不准一个人刷太久。"), emergency_passphrase: z.string().default(""), emergency_unlock_minutes: z.number().int().min(1).max(60).default(5), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", duration_minutes = 30, mode = "medium", reason, message, emergency_passphrase = "", emergency_unlock_minutes = 5, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        const locked_until_ms = Date.now() + Math.round(duration_minutes * 60000);
    ...
    server.tool("unlock_app", "应用门禁：旧版兼容工具名。结束某个App当前门禁，等同 end_screen_break。", { app: z.string().default(""), package: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("temporary_unlock_app", "应用门禁：旧版兼容工具名。临时放行一个正在门禁中的App。", { app: z.string().default(""), package: z.string().default(""), minutes: z.number().min(0.1).max(240).default(10), allow_type: z.string().default("real_time"), max_window_minutes: z.number().min(1).max(480).default(30), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", minutes = 10, allow_type = "real_time", max_window_minutes = 30, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("extend_lock", "应用门禁：旧版兼容工具名。延长某个App的门禁时间。", { app: z.string().default(""), package: z.string().default(""), minutes: z.number().min(0.1).max(1440).default(10), reason: z.string().default(""), message: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", minutes = 10, reason = "", message = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
    ...
    server.tool("deny_unlock_request", "应用门禁：旧版兼容工具名。拒绝这次恢复申请。", { app: z.string().default(""), package: z.string().default(""), message: z.string().default("这次先不放行，回来找陪伴对象。"), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", message = "这次先不放行，回来找陪伴对象。", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => {
        return gateCommand({ action: "deny_unlock_request", app, package: pkg, device_id, message, payload: { app, package: pkg, message } }, wait_seconds);
    });

    server.tool("list_lockable_apps", "应用门禁：旧版兼容工具名。列出可作为门禁对象的已安装App。", { max: z.number().int().min(10).max(200).default(80), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ max = 80, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "list_lockable_apps", max, device_id, payload: { max } }, wait_seconds));

    server.tool("add_locked_app", "应用门禁：旧版兼容工具名。把一个App加到可管理名单。", { alias: z.string(), package: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ alias, package: pkg, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "add_locked_app", app: alias, alias, package: pkg, device_id, payload: { alias, app: alias, package: pkg } }, wait_seconds));

    server.tool("remove_locked_app", "应用门禁：旧版兼容工具名。把一个App从可管理名单移除。", { app: z.string().default(""), package: z.string().default(""), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "remove_locked_app", app, package: pkg, device_id, payload: { app, package: pkg } }, wait_seconds));

    server.tool("set_emergency_passphrase", "应用门禁：旧版兼容工具名。为某个App当前门禁设置/更新紧急口令。", { app: z.string().default(""), package: z.string().default(""), passphrase: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", passphrase, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "set_emergency_passphrase", app, package: pkg, device_id, passphrase, emergency_passphrase: passphrase, emergencyPassphrase: passphrase, payload: { app, package: pkg, passphrase, emergency_passphrase: passphrase, emergencyPassphrase: passphrase } }, wait_seconds));

    server.tool("list_screen_break_apps", "屏幕休息：让手机列出可作为屏幕休息对象的已安装App，排除电话、设置、掌心窗和已配置陪伴目标应用等保护项。", { max: z.number().int().min(10).max(200).default(80), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ max = 80, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "list_screen_break_apps", max, device_id, payload: { max } }, wait_seconds));

    server.tool("add_screen_break_app", "屏幕休息：把一个App加到可管理名单。", { alias: z.string(), package: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ alias, package: pkg, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "add_screen_break_app", app: alias, alias, package: pkg, device_id, payload: { alias, app: alias, package: pkg } }, wait_seconds));

    server.tool("set_screen_break_passphrase", "屏幕休息：为某个App当前休息状态设置/更新紧急口令。", { app: z.string().default(""), package: z.string().default(""), passphrase: z.string(), device_id: z.string().default(DEFAULT_DEVICE), wait_seconds: z.number().int().min(3).max(20).default(8) }, async ({ app = "", package: pkg = "", passphrase, device_id = DEFAULT_DEVICE, wait_seconds = 8 }) => gateCommand({ action: "set_screen_break_passphrase", app, package: pkg, device_id, passphrase, emergency_passphrase: passphrase, emergencyPassphrase: passphrase, payload: { app, package: pkg, passphrase, emergency_passphrase: passphrase, emergencyPassphrase: passphrase } }, wait_seconds));
...
app.use((req, res, next) => {
    ...
    res.setHeader("Access-Control-Allow-Headers", ["Content-Type", "Authorization", "MCP-Protocol-Version", "MCP-Session-Id", "Mcp-Session-Id", "Last-Event-ID", "Accept"].join(","));
    res.setHeader("Access-Control-Expose-Headers", ["MCP-Session-Id", "Mcp-Session-Id", "WWW-Authenticate"].join(","));
...
app.get("/", (req, res) => res.type("text/plain").send("掌心窗 unified MCP is running. Use /mcp for Streamable HTTP, or /sse for SSE."));
